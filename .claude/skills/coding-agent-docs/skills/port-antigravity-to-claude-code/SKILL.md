...
the `coding-agent-docs:claude-code-docs` skill for the current Claude Code file format and
field names, and `coding-agent-docs:antigravity-docs` for the source. Do not migrate from
...
- **Ground target details in `coding-agent-docs:claude-code-docs`.** When the mapping is thin,
