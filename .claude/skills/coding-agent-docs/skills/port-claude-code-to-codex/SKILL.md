...
the `coding-agent-docs:codex-docs` skill for the current Codex file format and
field names, and `coding-agent-docs:claude-code-docs` for the source. Do not migrate from
...
- **Ground target details in `coding-agent-docs:codex-docs`.** When the mapping is thin,
